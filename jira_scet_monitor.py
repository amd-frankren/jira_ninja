...
- Requires EXTERNAL_JIRA_TOKEN from environment (missing => raise error).
...
def require_jira_base_url() -> str:
    """Read EXTERNAL_JIRA_URL from environment. Raise on missing base URL."""
...
def require_jira_token() -> str:
    """Read EXTERNAL_JIRA_TOKEN from environment. Raise on missing token."""
...
class JiraScetMonitor:
    ...
    def __init__(
        ...
        self.base_url = (base_url or require_jira_base_url()).rstrip("/")
        self.project_key = project_key
        self.token = require_jira_token()
        self.poll_interval_seconds = poll_interval_seconds
        self.max_results = max_results
        self.last_checked = initial_since or datetime.now(timezone.utc)

        print(
            f"[info] Jira monitor started: base={self.base_url}, "
            f"project={self.project_key}, interval={self.poll_interval_seconds}s"
        )
