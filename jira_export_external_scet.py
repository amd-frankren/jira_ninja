...
Purpose:
1) Called by `main.py`: when Jira ticket create/update events are detected, download full ticket data to local JSON.
2) Standalone debugging: manually specify `--browse-url` to download a single ticket.

Notes:
- Exported data includes:
  - issue body (with expand)
  - paginated comments / worklogs / changelog data (best effort to fetch all)

python jira_export_external_scet.py --browse-url [SCET Ticket URL]
...
def fetch_paginated(url: str, token: str, item_keys: List[str], page_size: int = 100) -> Dict[str, Any]:
    """
    Fetch paginated data from common Jira endpoints.
    Typical pagination fields:
      - startAt, maxResults, total
      - Array field may be values / comments / worklogs / histories
    """
    start_at = 0
    all_items: List[Any] = []
...
def fetch_issue_data(issue_key: str, token: str) -> Dict[str, Any]:
    ...
    if not changelog_fetched:
        result["warnings"].append(
            "Changelog pagination endpoint unavailable; kept data from issue.expand.changelog when present."
        )
...
def export_issue_to_file(
    issue_key: str,
    token: str = "",
    output_dir: str = DEFAULT_OUTPUT_DIR,
) -> str:
    """
    Download a specific issue and save it to output_dir/{issue_key}.json
    token is optional: if omitted, read from environment variable EXTERNAL_JIRA_TOKEN.
    ...
    resolved_token = (token or "").strip() or resolve_jira_token("")
    data = fetch_issue_data(safe_issue_key, resolved_token)
...
def export_issue_by_browse_url(browse_url: str, token: str = "", output_dir: str = DEFAULT_OUTPUT_DIR) -> str:
    """
    Download a ticket via browse URL and save it to local JSON.
    token is optional: if omitted, read from environment variable EXTERNAL_JIRA_TOKEN.
    """
...
def resolve_jira_token(cli_token: str) -> str:
    ...
    1) Prefer --token
    2) If omitted, read from environment variable
    """
    ...
    raise RuntimeError(
        f"Missing Jira token. Provide via --token or set environment variable: {ENV_EXTERNAL_JIRA_TOKEN}"
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download one Jira ticket and save as JSON")
    parser.add_argument("--browse-url", required=True, help="Jira issue browse URL")
    parser.add_argument("--token", default="", help="Jira Bearer token (optional; read from env if omitted)")
